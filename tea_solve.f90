  ...
  DO c=1,chunks_per_task
    ...
    IF(chunks(c)%task.EQ.parallel%task) THEN
      ...
      IF(tl_use_cg .OR. tl_use_chebyshev .OR. tl_use_ppcg) THEN
        ! All 3 of these solvers use the CG kernelsand this is also called for PETSc to get the initial residual
        IF(use_fortran_kernels) THEN
...
!======================================================
    ...
    IF(use_PETSC_kernels) THEN
        ...
        ! Set up the RHS
        CALL setupRHS_petsc(1,rx,ry,rz)
        ! Set up the Solution vector
        CALL setupSol_petsc(1,rx,ry,rz)
        
        ! Set up the Matrix
        CALL setupMatA_petsc(1,rx,ry,rz)
        ! Solve (as specified we use a command line approach to deifne solver/prec etc
